    ...
    @once
        <style>
            @media print {
                ...
                #flash-print-area, #flash-print-area * { visibility: visible !important; }
                #flash-print-area { 
                    position: absolute; 
                    left: 0; 
                    top: 0; 
    ...
    <flux:card class="space-y-6">
        ...
        <div>
            ...
            <flux:text class="mt-1">
                Simulation par batches fixes (unites / batch et kg huiles / batch). Donnees non persistantes.
            </flux:text>
        </div>
        ...
        <div class="space-y-3">
            @foreach ($this->lines as $index => $line)
                <div wire:key="line-{{ $line['line_key'] }}" class="flex gap-3">
                    {{-- Product Select --}}
                    <flux:select
                        wire:key="product-select-{{ $line['line_key'] }}"
                    ...
                    >
                        ...
                        @foreach ($this->productOptions as $productId => $label)
                            <flux:select.option wire:key="product-option-{{ $line['line_key'] }}-{{ $productId }}" value="{{ $productId }}">{{ $label }}</flux:select.option>
                        @endforeach
                    </flux:select>

                    {{-- Quantity Input --}}
                    <flux:input
                        wire:key="desired-units-{{ $line['line_key'] }}"
                    ...
                    {{-- Batch Preset Select (conditional) --}}
                    @if (! empty($line['product_id']))
                        <flux:select
                            wire:key="batch-preset-{{ $line['line_key'] }}"
                        ...
                        >
                            ...
                            @foreach ($this->getBatchPresetOptionsForLine($index) as $presetId => $label)
                                <flux:select.option wire:key="preset-option-{{ $line['line_key'] }}-{{ $presetId }}" value="{{ $presetId }}">{{ $label }}</flux:select.option>
                            @endforeach
                        </flux:select>
                    @else
                        <div class="w-5/12 flex h-10 items-center rounded-lg border border-zinc-200 bg-zinc-50 px-3 text-sm text-zinc-400">
        ...
        <div class="flex items-center gap-2 print:hidden">
            <flux:button wire:click="addLine" variant="filled">
                Ajouter un produit
            </flux:button>
            
            <flux:button wire:click="recalculate" variant="primary">
                Recalculer
            </flux:button>
            
            <flux:button type="button" onclick="window.print()" variant="ghost">
                Print
            </flux:button>
        </div>
    ...
    <flux:card class="mt-6">
        ...
        <div class="mt-3 grid gap-3 md:grid-cols-4 lg:grid-cols-7">
            ...
            <div class="rounded-lg border border-zinc-200 p-3">
                ...
                <div class="text-lg font-semibold">
                    {{ number_format((float) ($this->totals['total_desired_units'] ?? 0), 0, ',', ' ') }}
                </div>
            </div>
            ...
            <div class="rounded-lg border border-zinc-200 p-3">
                ...
                <div class="text-lg font-semibold">
                    {{ number_format((float) ($this->totals['total_produced_units'] ?? 0), 0, ',', ' ') }}
                </div>
            </div>
            ...
            <div class="rounded-lg border border-zinc-200 p-3">
                ...
                <div class="text-lg font-semibold">
                    {{ number_format((float) ($this->totals['total_extra_units'] ?? 0), 0, ',', ' ') }}
                </div>
            </div>
            ...
            <div class="rounded-lg border border-zinc-200 p-3">
                ...
                <div class="text-lg font-semibold">
                    {{ number_format((float) ($this->totals['total_batches'] ?? 0), 0, ',', ' ') }}
                </div>
            </div>
            ...
            <div class="rounded-lg border border-zinc-200 p-3">
                ...
                <div class="text-lg font-semibold">
                    {{ number_format((float) $this->totals['total_batch_kg'], 3, ',', ' ') }} kg
                </div>
            </div>
                ...
                <tbody>
                    @forelse ($this->ingredientTotals as $ingredient)
                        <tr class="border-b border-zinc-100">
                            ...
                            <td class="py-2 pr-4">
                                {{ number_format((float) $ingredient['required_quantity'], 3, ',', ' ') }} 
                                {{ $ingredient['base_unit'] }}
                            </td>
                            <td class="py-2 pr-4">
                                {{ number_format((float) $ingredient['unit_price'], 2, ',', ' ') }}
                            </td>
                            <td class="py-2 font-semibold">
                                {{ number_format((float) $ingredient['estimated_cost'], 2, ',', ' ') }}
                            </td>
                        </tr>
                    @empty
                        <tr>
                    ...
                    <tbody>
                        @foreach ($productSummaries as $line)
                            <tr class="border-b border-zinc-100">
                                ...
                                <td class="py-2 pr-4">
                                    {{ number_format((float) $line['desired_units'], 0, ',', ' ') }}
                                </td>
                                <td class="py-2 pr-4">
                                    {{ number_format((float) $line['produced_units'], 0, ',', ' ') }}
                                </td>
                                <td class="py-2 pr-4 font-semibold">
                                    {{ number_format((float) $line['extra_units'], 0, ',', ' ') }}
                                </td>
                                <td class="py-2 pr-4">
                                    {{ number_format((float) $line['batches_required'], 0, ',', ' ') }}
                                </td>
                                <td class="py-2">
                                    {{ number_format((float) $line['oils_kg'], 3, ',', ' ') }}
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
                    ...
                    <tbody>
                        @foreach ($this->productLines as $line)
                            <tr class="border-b border-zinc-100">
                                ...
                                <td class="py-2 pr-4">
                                    {{ number_format((float) $line['units_per_batch'], 0, ',', ' ') }}
                                </td>
                                <td class="py-2 pr-4">
                                    {{ number_format((float) $line['batches_required'], 0, ',', ' ') }}
                                </td>
                                ...
                                <td class="py-2 pr-4">
                                    {{ number_format((float) $line['batch_size_kg'], 3, ',', ' ') }}
                                </td>
                                <td class="py-2 pr-4">
                                    {{ number_format((float) $line['oils_kg'], 3, ',', ' ') }}
                                </td>
                                <td class="py-2 font-semibold">
                                    {{ number_format((float) $line['cost_per_unit'], 4, ',', ' ') }}
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
